...
void display_character(int columns, int lines, int l, int c)
{
    if (lines == 1 || columns == 1) {
        ...
        display_content(columns, lines, l, c);
    }
}

void display_content(int columns, int lines, int l, int c)
{
    if (l > 0 && l < lines-1) {
        if (c == 0 || c == columns-1) {
            my_putchar('*');
...
{
    if (l == 0 && c == 0 || l == lines-1 && c == columns-1) {
        my_putchar('/');
    }else if (l == 0 && c == columns-1 || l == lines-1 && c == 0) {
        my_putchar('\\');
    }else {
        my_putchar('*');
    }
